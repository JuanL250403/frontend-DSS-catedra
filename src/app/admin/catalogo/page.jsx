...
export default function Catalogo() {
    ...
    const cargarJuegos = async () => {
        setCargando(true)
        await peticiones.get("videojuegos").then((r) => {
    ...
    const buscarCategoria = async () => {
        ...
        setCargando(true)
        await peticiones.get('videojuegos', {
            params: {
                categoria: cat,
            }
        }).then((r) => {
    ...
    const verDetalles = (id) => {
        router.push(`/admin/catalogo/detalle/${id}`)
    }

    const verEdicion = (id) => {
        router.push(`/admin/catalogo/editar/${id}`)
    }

    const cambiarEstado = async (id) => {
        await peticiones.delete(`videojuegos/${id}`).then(async () => {
            showToast.success("Estado de videojuego actualizado")
            if (cat) {
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-12">
                ...
                <div>
                    ...
                    <p className="text-slate-400 mt-3 text-lg">
                        Estos son todo los Videojuegos registrados.
                    </p>

                    <button onClick={() => router.push("/admin/catalogo/crear")} className="bg-cyan-500 hover:bg-cyan-600 px-8 py-4 rounded-2xl text-lg font-bold transition">
                        Agregar Videojuego
                    </button>
                </div>
