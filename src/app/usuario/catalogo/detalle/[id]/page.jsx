...
export default function DetalleJuego() {
    const { agregarProducto, compra } = useCarrito()
    ...
    const cargarProducto = async () => {
        setCargando(true)
        await peticiones.get(`videojuegos/activos/${id}`).then((r) => {
            const j = r.data.data
            setJuego(j)
        })
        setCargando(false)
                        ...
                        <div className="space-y-4">
                            {juego.cantidad !== 0 ?
                                !juegoExistente() ?
                                    ...
                                    <button onClick={() => agregarProducto(juego)} className="w-full bg-cyan-500 hover:bg-cyan-600 py-5 rounded-2xl text-xl font-bold transition">
                                        Agregar al carrito
                                    </button>
                                    :
                                    <p className="w-full text-center py-5 rounded-2xl text-xl font-bold transition">
                                        En el carrito
                                    </p>
